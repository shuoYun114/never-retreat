...
function createStore(db,clock=()=>Date.now()){
 ...
 const user=n=>{const u=db.users[n];if(!u)throw Error('账号不存在');u.owned||={};u.loadouts||={};u.credits=Number(u.credits)||0;return u;};
 // 兵种自带 or 已购买，才算能用这把枪
 ...
 return {create,view,award,purchase,equip,canUse,loadout,newSession,sessionUser,dropSession,pruneSessions};
}
